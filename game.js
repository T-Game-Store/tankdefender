...
const IMG_PLAYER = "tank.png",
  IMG_ENEMY = "quai.png",
  IMG_ENEMY_TANK = "quai_trau.png",
  IMG_ENEMY_DASH = "quai_nhanh.png",
  IMG_ENEMY_BOMB = "quai_bom.png",
  IMG_EXPLOSION = "explosion.png",
  IMG_BOSS = "king_kong.png",
  IMG_BOSS_BULLET = "king_kong_bullet.png",
  IMG_SKELETON = "skeleton.png",
  IMG_SKELETON_BULLET = "skeleton_bullet.png";
...
const SKILL_G_NORMAL_URL = "skillshot1.mp3";
const skillGNormalSound = new Audio(SKILL_G_NORMAL_URL);

const SKILL_G_FROZEN_URL = "freeze_skill.mp3";
const skillGFrozenSound = new Audio(SKILL_G_FROZEN_URL);
...
const stick = document.getElementById("joystickStick"),
  ...
  skillBtn = document.getElementById("skillButton"),
  joystickZone = document.getElementById("joystickZone"),
  fireButtonZone = document.getElementById("fireButtonZone"),
...
function animate() {
  ...
  if (skillCooldown > 0) {
    skillCooldown--;
    document.getElementById("skillCooldownOverlay").style.height =
      (skillCooldown / 600) * 100 + "%";
    document.getElementById("skillTimerText").innerText =
      Math.ceil(skillCooldown / 60) + "s";
  } else {
    ...
    bullets.forEach((b) => {
      ...
      if (
        ...
        if (b.isFreezing === true) {
          ...
          boss.hp -= damage;
          b._dead = true;
          document.getElementById("bossHPBar").style.width =
  ...
  if (
    enemiesSpawned < config.enemiesTarget &&
    spawnTimer % config.spawnRate === 0
  ) {
    const type = getSpawnType(currentLevel);
    try {
      if (type === "skeleton") {
      ...
      console.error("Lỗi spawn quái:", e);
    }
  }

  enemies.forEach((en) => {
    if (!en || en._dead) return;
    en.update();
    en.draw();

    bullets.forEach((b) => {
      if (b._dead || en._dead || en.type === "bullet") return;
      if (
        b.x > en.x &&
        b.x < en.x + en.width &&
        b.y > en.y &&
        ...
        if (b.isFreezing === true) {
          ...
          if (!b.isPiercing) b._dead = true;
          else b.hitEnemies.push(en);

          if (en.hp <= 0) {
            if (en.type === "bomb") {
              document.body.classList.add("shake");
              setTimeout(() => document.body.classList.remove("shake"), 200);
              enemies.forEach((otherEn) => {
                if (
                  otherEn &&
                  otherEn !== en &&
                  !otherEn._dead &&
                  ...
                  let dist = Math.sqrt(
                    (en.x - otherEn.x) ** 2 + (en.y - otherEn.y) ** 2,
                  );
                  if (dist < 180) {
                    otherEn.hp -= 5;
                    if (otherEn.hp <= 0) {
                      ...
                      explosions.push(
                        new Explosion(
                          otherEn.x + otherEn.width / 2,
                          otherEn.y + otherEn.height / 2,
                          otherEn.width,
                        ),
                      );
                    }
    ...
    let dP = Math.sqrt(
      (en.x + en.width / 2 - player.x) ** 2 +
        (en.y + en.height / 2 - player.y) ** 2,
    );
  ...
  bullets = bullets.filter((b) => b && !b._dead);
  enemies = enemies.filter((en) => en && !en._dead);
  explosions = explosions.filter((ex) => ex && ex.life > 0);
}
...
function initGame(level) {
  ...
  lives = config.lives || 5;
  enemiesLeft = config.enemiesTarget || 20;
  enemiesSpawned = 0;
  spawnTimer = 0;
  skillCooldown = 0;
  gameActive = true;
...
function showMenu() {
  ...
  ["levelSelect", "endScreen", "gameHUD", "pauseMenu", "skinMenu"].forEach(
    (id) => {
