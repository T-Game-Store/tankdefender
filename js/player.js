class Player {
    ...
    draw() {
        ...
        ctx.fillStyle = grad; ctx.globalCompositeOperation = 'screen'; ctx.fillRect(0, 0, canvas.width, canvas.height);
        ctx.restore();
        ctx.save();
    ...
    update() {
        ...
        if (joystickData.x !== 0 || joystickData.y !== 0) { moveX = joystickData.x; moveY = joystickData.y; }
        if (moveX !== 0 || moveY !== 0) {
            this.angle = Math.atan2(moveY, moveX) + Math.PI / 2;
            let length = Math.sqrt(moveX * moveX + moveY * moveY);
            this.x += (moveX / length) * this.speed; this.y += (moveY / length) * this.speed;
        }
        this.x = Math.max(50, Math.min(canvas.width - 50, this.x));
        this.y = Math.max(canvas.height * 0.35, Math.min(canvas.height - 70, this.y));
